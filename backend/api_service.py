...
def get_business_units():
    ...
    if not conn:
        ...
        return jsonify({
            ...
            'hyperparameters': f"Refresh Interval: {random.choice([5, 10, 15])} minutes, Max Units: {random.choice([50, 100, 200])}"
        })
    conn = get_db()
    if not conn:
...
@limiter.limit("5 per minute")
def get_errors():
    """Enhanced errors endpoint with better frontend compatibility"""
    conn = get_db()
    if not conn:
        ...
        # Generate more realistic error patterns
...
def get_mode1_xai():
    conn = get_db()
    if not conn:
        print("Using mock XAI data: DB connection failed")
        return jsonify({
            'id': f"XAI-{generate_alphanum_id()}",
            'text': 'Model shows 85% precision and 92% recall for Class A predictions',
            'status': 'success'
        })
    try:
        cursor = conn.cursor()
        cursor.execute('SELECT explanation FROM Mode1XAI')
        row = cursor.fetchone()
        if not row:
            return jsonify({
                'id': f"XAI-{generate_alphanum_id()}",
        ...
        return jsonify({
            'id': f"XAI-{generate_alphanum_id()}",
            'text': f"Error retrieving XAI data: {str(e)}",
            'status': 'error'
        })
    finally:
        conn.close()
...
@limiter.limit("30 per minute")  # Increased for dashboard needs
def get_mode2_metrics(business_unit=None):
    # Initialize database connection
    conn = get_db()
    if not conn:
        print("Using enhanced mock data: DB connection failed")
        conn = None  # Ensure conn is set to None for mock data handling
    ...
    def generate_alphanum_id():
        chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'
        return ''.join(random.choice(chars) for _ in range(6))
    # Generate discrete drift values (0,1,2) with realistic patterns
    base_drift = random.randint(0, 1)  # Start with 0 or 1
    drift_trend = random.choice(['increasing', 'decreasing', 'stable'])
    # Enhanced mock data with business context
    business_context = {
        'CCS': {
...
def get_mode3_metrics():
    """Unified endpoint for mode3 metrics with frontend-compatible response"""
    if request.args.get('mode') != 'mode3' and not request.path.endswith('/3'):
        return jsonify({"error": "Invalid mode parameter"}), 400
    
    # Generate complete dataset in frontend-expected format
    data = {
        "kpis": [
